<template>
    <div class="container-fluid  mt_12">
        <div class="card onewhitebg rounded border-0 ">
           <div class="card-body ">
                <h4 class="coman_main_heading border-bottom pb-2 mb-3">File Claim</h4>
                ...
                <div v-if="currentStep == 2" class="step">
                    <div class="row">
                        <div class="col-12">
                            ...
                            <small v-if="numberOfDevices > 50 && !validationErrors.numberOfDevices"><ErrorMessage :msg="`You can only add up to 50 devices.`"></ErrorMessage></small>
                            <!-- <small v-if="numberOfDevices < 1"><ErrorMessage :msg="`Please enter a valid number of devices`"></ErrorMessage></small> -->
                            <small v-if="validationErrors.numberOfDevices" ><ErrorMessage :msg="validationErrors.numberOfDevices[0]"></ErrorMessage></small>
                        </div>
                    </div>
                    <div v-for="(device, index) in devices" :key="index" class="device-info  onewhitebg mt-3 card">
                        <div class="d-flex flex-wrap card-header align-items-center ">
                            <div class="col-md-11">
                                <h4 class=" my-0  def_18_size themetextcolor">Device {{ index + 1 }}</h4>
                            </div>
                            <div class="col-md-1 d-flex justify-content-end">
                                <img @click="removeDeviceList(index)" v-if="devices.length > 1" :src="deleteIc" width="28" height="28">
                            </div>
                        </div>
                        <div class="container-fluid">
                            <div class="row mt-2">
                                ...
                                <div class="col-12 col-md-6 col-lg-6 mb-3">
                                    ...
                                    <small v-if="validationErrors[`devices.${index}.selectedDevice`]" ><ErrorMessage :msg="validationErrors[`devices.${index}.selectedDevice`][0]"></ErrorMessage>  </small>
                                </div>
                                <div class="col-12 col-md-6 col-lg-6 mb-3">
                                    ...
                                    <small v-if="validationErrors[`devices.${index}.claimReason`]" ><ErrorMessage :msg="validationErrors[`devices.${index}.claimReason`][0]"></ErrorMessage> </small>
                                </div>
                            </div>
                            ...
                            <div class="row">
                                <div class="col-md-12 mb-3">
                                    ...
                                    <small v-if="validationErrors[`devices.${index}.claimDetails`]"  ><ErrorMessage :msg="validationErrors[`devices.${index}.claimDetails`][0]"></ErrorMessage></small>
                                </div>
                            </div>
                        </div>
                    </div>
                   <!--  <button type="button" class="btn bg-dark text-white mt-4 d-flex align-items-center gap-10" @click="addDeviceList">
                ...
                <!-- Step 3: Shipping and Payment Details -->
                <div v-if="currentStep == 3" class="step  device-info">
                                ...
                                <table class="table">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>Claim Reason</th>
                                            <th>Claim Details</th>
                                            <th v-if="!isHodOrDirector">Claim Fee</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr v-for="(device, index) in devices.filter((d, i, arr) => arr.findIndex(x => x.selectedDevice?.id == d.selectedDevice?.id) == i)" :key="index">
                                            ...
                                            <td v-if="!isHodOrDirector">${{ device.claimChargesAmount ?? 0 }}</td>
                                        </tr>
                                    </tbody>
                                    <tfoot v-if="!isHodOrDirector">
                                        <tr><td colspan="4" class="text-end"><strong>Total</strong></td>
                                            <td><strong>${{ claimDeductPrice.toFixed(2) }}</strong></td>
                                        </tr>
                                    </tfoot>
                                </table>
                            </div>
                    ...
                    <div class="mb-3 bg-white p-3 rounded border">
                        ...
                        <div v-if="!isHodOrDirector" class="mb-3">
                            ...
                            <select class="form-control" v-model="shippingOption" @change="handleShippingOption" :class="['form-control',{ 'is-invalid': validationErrors.shippingOption },]">
                            ...
                            <small v-if="validationErrors.shippingOption" ><ErrorMessage :msg="validationErrors.shippingOption[0]"></ErrorMessage></small>
                        </div>
                ...
                <div class="d-flex justify-content-between mt-4">
                    <a v-if="currentStep == 1" :href="$userAppUrl + 'sdcsmuser/user-track-claims'" class="btn btn-secondary me-2">Cancel</a>
                    <button v-if="currentStep == 2 || currentStep == 3" class="btn customm_reset_btn  me-2" @click="prevStep" :disabled="disabledButton && currentStep == 3">Back</button>
                    <button v-if="currentStep == 1 || currentStep == 2" class="btn bg_blue text-white " @click="nextStep">Next</button>
                    <button v-if="currentStep == 3" class="btn bg_blue text-white" @click="submitForm" :disabled="disabledButton && currentStep == 2">Submit</button>
                </div>
            </div>
        </div>
    </div>
    ...
    export default {
            ...
            return {
                ...
                numberOfDevices: 1,
                    ...
                    devices : [{
                        ...
                        claimDetails: '',
                        claimreasonsdata: [],
                        claimChargesAmount: 0,
                    }],

                    /** To manage the address edit(enabled or disabled) */
                    isAddressEdit: 0,

                    /** To manage the first step data */
                    firstName: '',
                    lastName: '',
                    phone: '',
                    email: '',
                ...
                /** To manage the total number of devices */
                totalDevices: 0,
                shippingAmount: '',
        ...
        watch: {
            currentStep(newStep) {
                ...
                if (newStep == 3 && !this.isHodOrDirector) {
                    this.grandTotal = Number(this.claimDeductPrice) + Number(this.shippingAmount);
        ...
        methods: {
            ...
            initializeCardElement() {
                this.$nextTick(() => {
                    if (!this.isHodOrDirector && this.grandTotal > 0 && this.shippingOption) {
            ...
            /** Get current user data to show the fields pre-filled*/
            async getUserData(){
                ...
                try{
                    ...
                    if(response.data.success == true){
                        if(response.data.user_Details){
                            ...
                            this.firstName = response.data.user_Details.first_name;
                            this.lastName = response.data.user_Details.last_name;
                            this.email = response.data.user_Details.email;
                            this.phone = response.data.user_Details.phone;
                            this.cardNumber = response.data.user_Details.card_number;
                            this.cardHolderName = response.data.user_Details.card_holder_name;
            ...
            async nextStep(){
                ...
                if(this.currentStep == 1){
                    ...
                    if(!this.email){
                        this.validationErrors.email = ['Please enter email id.'];
                        validationPassed = false;
                    }
                ...
                const uniqueDevices = this.devices.filter(
                    (d, i, arr) => arr.findIndex(x => x.selectedDevice?.id == d.selectedDevice?.id) == i
                );

                this.claimDeductPrice = uniqueDevices.reduce((sum, device) => {
                    const fee = parseFloat(device.claimChargesAmount || 0);
                    return sum + (isNaN(fee) ? 0 : fee);
                }, 0);



                /* this.claimDeductPrice = this.devices.reduce((sum, device) => {
                        const fee = parseFloat(device.claimChargesAmount || 0);
                        return sum + (isNaN(fee) ? 0 : fee);
                }, 0); */
            ...
            async submitForm(){
                ...
                /*
                if(!this.phoneNumber){
                    this.validationErrors.phoneNumber = ['Please enter phone number.'];
                    validationPassed = false;
                }
                ...
                */
                /* if(!this.cardNumber){
                    this.validationErrors.cardNumber = ['Please enter card number.'];
                    validationPassed = false;
                } */
                if(!this.cardHolderName && !this.isHodOrDirector  && this.shippingOption && this.grandTotal > 0){
                    this.validationErrors.cardHolderName = ['Please enter card holder name.'];
                    validationPassed = false;
                }
                ...
                try {
                    ...
                    let response = await axios.post(`${this.$userAppUrl}sdcsmuser/fileclaims/store`, {

                        /** User details */
                        firstName: this.firstName,
                        lastName: this.lastName,
                        phone: this.phone.replace(/\D/g, ''), // Remove non-digit characters
                        email: this.email,
                        ...
                        // amount: this.shippingOptions.find((so,index) => so.id == this.shippingOption)?.price,
                    ...
                    if (response.data.success == true) {
                        ...
                        this.country = '';
                        hide_ajax_loader();
                        setTimeout(() => (location.href = `${this.$userAppUrl}sdcsmuser/user-track-claims`),2000);
            ...
            handleDeviceSelect(index) {
                ...
                this.getDeductiblePrice(device.selectedDevice.id);
            },

            /** Handle Remove Device */
            handleRemoveDevice(index) {
                this.getClaimReasons(null, index);
            },

            /** To get the devices data */
            async getDevices(name = this.searchModelText){
                show_ajax_loader();
                ...
                try {
                    let response = null;
                    response = await axios.post(`${this.$userAppUrl}sdcsmuser/fileclaims/getdevices`, {
                        name: name,
                    });
            ...
            async getClaimReasons(deviceId, index) {
                ...
                try {
                    let response = await axios.get(`${this.$userAppUrl}sdcsmuser/claim-reasons?deviceId=${deviceId}`);
                    if (response.data.success == true) {
            ...
            /** To get Deductible price according to the device */
            async getDeductiblePrice(deviceId) {
                show_ajax_loader();
                try {
                    let response = await axios.get(`${this.$userAppUrl}sdcsmuser/fileclaims/device-plan?deviceId=${deviceId}`);

                    let fee = 0;
                    if (response.data.success == true) {
                        if(response.data.plan && response.data.plan.deductible_price){
                            fee = Number(response.data.plan.deductible_price).toFixed(2) ?? 0;
                            hide_ajax_loader();
                        } else {
                            fee = 0;
                            hide_ajax_loader();
                    ...
                    this.devices.forEach(device => {
                        if (device.selectedDevice?.id == deviceId) {
                            device.claimChargesAmount = fee;
                        }
                    });
                    hide_ajax_loader();
                } catch (error) {
            ...
            handleShippingOption() {
                ...
                const totalFee = Number(this.claimDeductPrice);
