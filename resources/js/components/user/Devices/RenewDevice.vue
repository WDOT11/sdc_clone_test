                            ...
                            <tr>
                                <th scope="col" style="width: 40px">
                                    <input type="checkbox" class="form-check-input" @change="toggleSelectAll($event)"
                                        :checked="allSelectedOnCurrentPage" :indeterminate="isIndeterminate" />
                                </th>
                            ...
                            <tr v-if="deviceData.length > 0" v-for="(data, index) in deviceData" :key="data.id"
                                ...
                                <td>
                                    <select v-model="deviceCoveragePlans[data.id]" class="form-control">
                                        <!-- @focus="handleSelectModel(data.device_model_id, data.org_id)"> -->
                                ...
                                <td>
                                    {{ data.device_type == 1 ? "Personal" : data.device_type == 2 ? "Organizational" : "" }}
                                </td>
...
export default {
    ...
    mounted() {
        this.deviceData.forEach(device => {
            ...
            this.handleSelectModel(device.device_model_id, device.org_id);
        });
    },
    watch: {
        deviceCoveragePlans: {
    ...
    methods: {
         ...
         * Plan Data
         */
        async handleSelectModel(modelId, orgId = null) {
            ...
            try {
                ...
                const response = await axios.post(`${this.$userAppUrl}sdcsmuser/renewal-devices/plan`, payload);
        ...
        async submitForm() {
            ...
            try {
                ...
                let response = await axios.post(`${this.$userAppUrl}sdcsmuser/renewal-devices/store`, payload);
