    ...
    <div class="container-fluid  mt_12">
        ...
        <div class="card border-0 rounded onewhitebg">
            <div class="card-body rounded   ">
                    ...
                    <table class="table table-bordered table-hover table_custom">
                        ...
                        <tbody>
                            <tr v-if="notificationData.length > 0" v-for="(notification, index) in notificationData" :key="notification.id">
                                <td class="ps-3">{{ (paginationData.current_page < 1 ? paginationData.current_page : paginationData.current_page - 1) *paginationData.per_page + index + 1}}
                                </td>
                                <td>{{ notification.message }}</td>
                                <td class="text-nowrap">{{ formatDate(notification.created_at) }}</td>
                                <td>
                                    <a class="globalviewbtn text-decoration-none  d-flex align-items-center themetextcolor gap-1 def_14_size fw-semibold" :href="notification.notification_for == 'user_added' ? '#': `${$userAppUrl}${notification.user_target_link}?openPopup=true${notification.device_id ? `&deviceId=${notification.device_id}` : ''}${notification.device_claim_id ? `&claimId=${notification.device_claim_id}` : ''}${notification.device_repair_id ? `&repairId=${notification.device_repair_id}` : ''}`"><img :src="viewIc" width="35" height="35"> <span class="viewtext_Gl">View</span></a>
                                </td>
                            </tr>
                            <tr v-else>
                ...
                <user-pagination v-if="paginationData && paginationData.last_page > 1" :pagination="paginationData" :offset="5" :paginate="getnotificationData"></user-pagination>
            </div>
        </div>
    ...
    methods: {
        ...
        async getnotificationData(page = 1) {
            try {
                ...
                let url = `${this.$userAppUrl}sdcsmuser/notifications?page=${page}`;
