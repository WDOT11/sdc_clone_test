<template>
  ...
  <div :class="notificationsvisible ? 'notificaton_bar_wrap onewhitebg  slideInRight  onopen_notification' : 'notificaton_bar_wrap onewhitebg  slideInRight'">
    <div class="notificaton_bar_inner h-100">
      <div class="card  h-100 bg-transparent border-0 notification_div">
        <div class="notification_listdataconent">
          <div  class="card-header  py-3 main_notif_title d-flex justify-content-between align-items-center    ">
            ...
            <svg xmlns="http://www.w3.org/2000/svg" version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink" width="22"
              height="22" x="0" y="0" viewBox="0 0 511.76 511.76" style="enable-background: new 0 0 512 512"
              xml:space="preserve" class="notification_closemaster" @click="hideNotification">
              <g>
                <path
        ...
        <div class="card-body h-100 py-0 notification_list px-0 m-0">
          <div class="d-flex justify-content-between align-items-center">
            <!-- <ul class="nav nav-pills mb-2 mt-2" id="pills-tab" role="tablist">
              <li class="nav-item pl-2" role="presentation">
                  <button class="nav-link py-1 def_14_size active rounded-pill d-flex gap-1 align-items-center" id="pills-home-tab" data-bs-toggle="pill" data-bs-target="#pills-home" type="button" role="tab" aria-controls="pills-home" aria-selected="true">All <span class="badge line_hight1 bg-danger rounded-pill">50+</span></button>
              </li>


            </ul> -->
            <!-- <a href="#" class="unreadbtn def_12_size text-decoration-underline">Mark as unread</a> -->
          </div>
          ...
          <ul v-if="notificationsdata.length > 0" class="notification_listIn">
            ...
            <!-- <a :href="notification.notification_for == 'user_added' ? '#': `${$userAppUrl}${notification.user_target_link}?openPopup=true${notification.device_id ? `&deviceId=${notification.device_id}` : ''}${notification.device_claim_id ? `&claimId=${notification.device_claim_id}` : ''}${notification.device_repair_id ? `&repairId=${notification.device_repair_id}` : ''}`"> -->
        ...
        <div class="card-footer border-top-0 d-flex justify-content-center py-2 modal-header">
            <a href="#" class="rounded-pill text-decoration-none def_14_size view_all" @click="markAsSeen">View All</a>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    /** Mark Seen to all */
    markAsSeen() {
      axios.get(`${this.$userAppUrl}sdcsmuser/notifications/markSeen`)
      .then(() => {
          /* optionally update UI or refetch notifications */
          window.location.href = `${this.$userAppUrl}sdcsmuser/notifications`;
      });
    },
    /** Mark as seen to particular only */
    handleNotificationClick(notification) {
      // Build the redirect URL only if it's not 'user_added'
      let targetUrl = '';
      if (notification.notification_for !== 'user_added') {
        targetUrl = `${this.$userAppUrl}${notification.user_target_link}?openPopup=true`;
      ...
      // Call API to mark the notification as seen
      axios.post(`${this.$userAppUrl}sdcsmuser/notification/single-markSeen`, {
