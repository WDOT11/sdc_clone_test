<template>
    ...
    <div v-if="!isLoading" class="card-body d-flex flex-column justify-content-between border_radious375 report_tab_inner">
        <div class="chartWrapper">
            <canvas ref="userPieChart" width="250" height="250"></canvas>
            <div class="center-text1">
            <div class="white_inner">
                <h2>Total</h2>
                <small>{{ formattedDigits(totalUsers) }}</small>
            </div>
            </div>
        </div>
        <div class="custom-legend">
            <div class="legend-item def_16_size">
                <span class="legend-color" style="background-color: #f4c542;"></span> Total Users
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        async getUsersData(year) {
            this.isLoading = true;
            try {
                let url = `${this.$userAppUrl}smarttiusadmin/reports/total-users?year=${year}`;
                ...
                if (response.data.success == true) {
                        ...
                        this.$nextTick(() => {
                            const canvas = this.$refs.userPieChart;
                            if (canvas) {
                                this.usersChart();
                            } else {
                                /* Loader */
