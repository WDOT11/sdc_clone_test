<template>
    ...
    <!-- Chart Container -->
    <div v-if="!isLoading"
        class="card-body d-flex h-100 flex-column justify-content-end px-3 border_radious375 report_tab_inner">
        <div class="chart-containerm">
            <canvas ref="deviceRepairChart"></canvas>
        </div>
    </div>
</template>
<script>
export default {
    ...
    methods: {
        ...
        async getDevicesData(year) {
            this.isLoading = true;
            try {
                let url = `${this.$userAppUrl}smarttiusadmin/reports/device-repair-month?year=${year}`;
                ...
                if (response.data.success == true) {
                    if (response.data.deviceData) {
                        ...
                        this.$nextTick(() => {
                        const canvas = this.$refs.deviceRepairChart;
                        if (canvas) {
                            this.devicesChart();
                        } else {
                            /* Loader */
