<template>
    ...
    <!-- Chart Container -->
    <div v-if="!isLoading"
        class="card-body d-flex flex-column h-100 justify-content-end px-3 border_radious375 p-2 report_tab_inner">
        <div class="chart-containerm">
            <canvas ref="deviceClaimChart"></canvas>
        </div>
    </div>
</template>
<script>
export default {
    ...
    methods: {
        ...
        async getDevicesData(year) {
            this.isLoading = true;
            try {
                let url = `${this.$userAppUrl}smarttiusadmin/reports/device-claim-month?year=${year}`;
                ...
                if (response.data.success == true) {
                    if (response.data.deviceData) {
                        ...
                        this.$nextTick(() => {
                        const canvas = this.$refs.deviceClaimChart;
                       if (canvas) {
                            this.devicesChart();
                        } else {
                            /* Loader */
