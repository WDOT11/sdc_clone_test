...
class AdminShippingOptionController extends Controller
{
    ...
    public function index(Request $request){
        ...
        if (!empty($request->page)) {
            return response()->json(["shippingOptionsData"=>$shippingOptions, "pagination"=> $pagination, "msg" => "Shipping Options Fetched.", "success" => true], 200);
        } else {
            return view('admin.adminshippingoptions.index', compact('shippingOptions', 'pagination'));
        }
    }
    ...
    public function store(Request $request){
        try{
            ...
            $validator = Validator::make($request->all(), [
                'name' => 'required|string|max:50|unique:shipping_options,name,Null,id,active,1,option_for,1,service_provider_id,' . $serviceProvider,
            ...
            if($shippingOption->wasRecentlyCreated){
                return response()->json(["msg" => "Shipping option added successfully.", "success" => true], 200);
            }else {
                return response()->json(["msg" => "Shipping option creation failed.", "success" => false], 200);
            }
        } catch (\Exception $e) {
    ...
    public function edit(string $id){
        if(!empty($id))
        {
            ...
            if(!empty($shippingOptions)){
                return response()->json(["editData" => $shippingOptions, "msg" => "Edit Shipping Option Data.", "success" => true], 200);
            } else {
    ...
    public function update(Request $request, $id){
        try {
            ...
            /* Validate to the requests */
            $validator = Validator::make($request->all(), [
                'name' => 'required|string|max:50|unique:shipping_options,name,' . $id . ',id,active,1,option_for,1,service_provider_id,' . $serviceProvider,
                'price' => 'required|numeric|min:0',
            ], [
            ...
            $shippingOption = ShippingOption::where('id', $id)->where('option_for', 1)->where('service_provider_id', $serviceProvider)->where('active', 1)->first();
            if (!empty($shippingOption)) {
                ...
                if (!empty($shippingOptionUpdate)) {
                    return response()->json(["msg" => "Shipping option updated successfully.", "success" => true], 200);
                } else {
                    return response()->json(["msg" => "Shipping option updation failed.", "success" => false], 200);
                }
            } else {
    ...
    private static function getShippingOptionsQuery($where = [], $fields = []){
        ...
        $serviceProvider = Session::get('service_provider');
        $query = ShippingOption::where('service_provider_id', $serviceProvider)->where('option_for', 1)->where('active', 1);
